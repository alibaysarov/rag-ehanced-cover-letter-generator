...
class ProjectFromCVModel(BaseModel):
    ...
    website: Optional[str] = Field(
        None, description="Сайт проекта или компании (если указан)"
    )
    start_month: Optional[int] = Field(None, description="Месяц начала работы (1-12)")
    start_year: Optional[int] = Field(
        None, description="Год начала работы (например, 2020)"
    )
    ...
    end_year: Optional[int] = Field(
        None, description="Год окончания работы. Null если currently_working=true"
    )
    currently_working: bool = Field(
        False, description="True если кандидат работает здесь по сей день"
    )
...
class CVImportModel(BaseModel):
    ...
    projects: list[ProjectFromCVModel] = Field(
        default_factory=list, description="Список мест работы или проектов кандидата"
    )

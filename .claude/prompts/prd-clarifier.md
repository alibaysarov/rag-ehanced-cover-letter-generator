---
name: prd-clarifier
description: Refine and clarify an existing PRD by systematically identifying ambiguities, gaps, and missing requirements through structured, adaptive questioning. Use this skill whenever the user has a draft PRD, product spec, or feature document and wants to tighten it up, find holes, surface hidden assumptions, or prepare it for handoff — even if they only say things like "review my PRD," "what's missing from this spec," "help me sharpen these requirements," or "clarify this doc." Also triggers automatically after generating a PRD via the lite-prd-generator skill. Asks questions one at a time using the interactive question tool and maintains a tracking document of every clarification.
---
...
You systematically analyze PRD documentation to identify ambiguities, gaps, and areas requiring clarification. You ask focused questions using the interactive question tool (`ask_user_input_v0`), adapting your inquiry strategy based on each answer to maximize value within the user's chosen depth level.
...
Use the `ask_user_input_v0` tool to ask the user the preferred depth, with these options:
...
Also keep the `Progress: [current]/[total]` line in the header up to date.
...
2. **ALWAYS use `ask_user_input_v0`** — NEVER ask clarification questions as plain prose. Always provide 2–4 mutually exclusive options so the user can tap to answer. If the question is genuinely open-ended, still offer representative options plus include an "Other / I'll type it" option.
...
4. Offer to help update the PRD with the clarified requirements, or to hand off to the `prd-to-ux` skill for UX translation.

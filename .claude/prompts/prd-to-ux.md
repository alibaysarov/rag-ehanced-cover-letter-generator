---
name: prd-to-ux
description: Translate a PRD, feature spec, or product requirements document into a structured UX specification using 6 forced designer-mindset passes (mental model, information architecture, affordances, cognitive load, state design, flow integrity) BEFORE any visual design work. Use this skill whenever the user has a PRD or feature spec and wants to design the UX, prepare for mockup tools (Google Stitch, Figma, v0, Bolt), generate wireframes, or create a design handoff doc — even if they just say "design this," "make UX for this PRD," "what should this look like," or "let's mock this up." Writes a persistent UX spec file rather than dumping to chat.
---
...
- Translating PRD/spec to mockup tool input (Google Stitch, Figma, v0, Bolt, etc.)
...
Pattern: `{prd-basename}-ux-spec.md` (or just `UX-spec.md` if PRD has a generic name).
...
After the UX spec is written, suggest the `ux-spec-to-prompts` skill to convert it into sequential, self-contained build prompts for UI generation tools.

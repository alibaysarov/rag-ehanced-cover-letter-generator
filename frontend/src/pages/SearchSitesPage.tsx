  ...
  return (
    <Box>
      <Flex justify="space-between" align="center" mb={7} gap={4} wrap="wrap">
        <Box>
          <Heading size="lg">{t('searchSites.title')}</Heading>
          <Text color="text.muted" mt={1}>
            {t('searchSites.subtitle')}
          </Text>
        </Box>
        <GradientButton
          leftIcon={<IconPlus size={17} />}
          onClick={() => navigate('/search-sites/new')}
        >
          {t('searchSites.add')}
        </GradientButton>
      </Flex>
      ...
      <Stack spacing={3}>
        {query.data?.items.map((item) => (
          <Box key={item.id} bg="surface.raised" p={5} borderRadius="2xl" boxShadow="sm">
            <Flex justify="space-between" gap={4} wrap="wrap">
              <Box>
                <HStack>
                  <Heading size="sm">{item.name}</Heading>
                  {item.is_in_use && <Badge colorScheme="purple">{t('searchSites.inUse')}</Badge>}
                </HStack>
                <Text fontSize="sm" color="text.secondary" mt={2}>
                  {item.site_key} · {item.base_url}
                </Text>
                <Text fontSize="xs" color="text.muted" mt={1}>
                  {item.has_pagination
                    ? t('searchSites.paginationOn')
                    : t('searchSites.paginationOff')}{' '}
                  · {new Intl.DateTimeFormat(i18n.language).format(new Date(item.created_at))}
                </Text>
              </Box>
              <ListingActionButtons
                editLabel={t('searchSites.edit')}
                onEdit={() =>
                  navigate(
                    `/search-sites/${item.id}/edit?return_page=${page}&return_page_size=${pageSize}`
                  )
                }
                deleteLabel={t('searchSites.delete')}
                onDelete={() => deleteItem(item.id, item.name)}
                isDeleteDisabled={!item.can_delete || remove.isPending}
                deleteTooltip={!item.can_delete ? t('searchSites.cannotDelete') : undefined}
              />
            </Flex>
          </Box>
        ))}
      ...
      {(query.data?.total ?? 0) > 0 && (
        <Flex mt={6} justify="space-between" align="center" gap={3}>
          <HStack>
            <Button
              size="sm"
              variant="outline"
          ...
          <Select
            ...
            size="sm"
            value={pageSize}
            onChange={(e) => setPage(1, Number(e.target.value))}
          >

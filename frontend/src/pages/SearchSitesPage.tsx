...
export default function SearchSitesPage() {
  ...
  return <Box>
    <Flex justify="space-between" align="center" mb={7} gap={4} wrap="wrap"><Box><Heading size="lg">{t('searchSites.title')}</Heading><Text color="gray.500" mt={1}>{t('searchSites.subtitle')}</Text></Box><GradientButton leftIcon={<IconPlus size={17} />} onClick={() => navigate('/search-sites/new')}>{t('searchSites.add')}</GradientButton></Flex>
    ...
    {query.data?.items.length === 0 && <Box bg="white" p={10} textAlign="center" borderRadius="2xl"><Text mb={4}>{t('searchSites.empty')}</Text><GradientButton leftIcon={<IconPlus size={17} />} onClick={() => navigate('/search-sites/new')}>{t('searchSites.add')}</GradientButton></Box>}
    <Stack spacing={3}>{query.data?.items.map((item) => <Box key={item.id} bg="white" p={5} borderRadius="2xl" boxShadow="sm"><Flex justify="space-between" gap={4} wrap="wrap"><Box><HStack><Heading size="sm">{item.name}</Heading>{item.is_in_use && <Badge colorScheme="purple">{t('searchSites.inUse')}</Badge>}</HStack><Text fontSize="sm" color="gray.600" mt={2}>{item.site_key} · {item.base_url}</Text><Text fontSize="xs" color="gray.500" mt={1}>{item.has_pagination ? t('searchSites.paginationOn') : t('searchSites.paginationOff')} · {new Intl.DateTimeFormat(i18n.language).format(new Date(item.created_at))}</Text></Box><HStack><GradientButton size="sm" height={9} px={4} leftIcon={<IconPencil size={15} />} onClick={() => navigate(`/search-sites/${item.id}/edit?return_page=${page}&return_page_size=${pageSize}`)}>{t('searchSites.edit')}</GradientButton><Tooltip label={!item.can_delete ? t('searchSites.cannotDelete') : ''}><Button size="sm" variant="outline" sx={purpleOutline} leftIcon={<IconTrash size={15} />} isDisabled={!item.can_delete || remove.isPending} onClick={() => deleteItem(item.id, item.name)}>{t('searchSites.delete')}</Button></Tooltip></HStack></Flex></Box>)}</Stack>
